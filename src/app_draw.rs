use crate::app_default::{
    Annotation, MAX_TEXTURE_SIZE, MouseSelectionRect, PointerSnapshot, PrimaryButtonTransition,
    ScreenshotApp, TextInputState, Tool,
};
...
impl ScreenshotApp {
        ...
        match transition {
            PrimaryButtonTransition::Pressed => {
                        ...
                        .is_some_and(|rect| rect.contains(position))
                {
                    ...
                    self.mouse_move_start = (position.x.round() as i32, position.y.round() as i32);
            ...
            PrimaryButtonTransition::Held => {
                if self.is_selecting {
                    ...
                    if let (Some(original), Some(desktop)) = (
                        ...
                        self.mouse_start = (moved.min.x.round() as i32, moved.min.y.round() as i32);
                        self.mouse_end = (moved.max.x.round() as i32, moved.max.y.round() as i32);
                        self.mouse_selection_rect = Some(MouseSelectionRect {
                            start: self.mouse_start,
                            end: self.mouse_end,
                        });
                    }
                        ...
                        .is_some_and(|rect| rect.contains(position))
                {
                    ...
                    annotation
                        .mouse_points
                        .push((position.x.round() as i32, position.y.round() as i32));
                }
            }
    ...
    pub(crate) fn draw_screens(&mut self, ui: &mut egui::Ui) {
                ...
                || self.screen_height > MAX_TEXTURE_SIZE as i32)
        {
            ...
            for (_x, _y, texture_handle) in &self.display_textures_split {
                ...
                if self.screen_width > MAX_TEXTURE_SIZE as i32 {
                    self.image_scale = texture_size.y / viewport_rect.max.y;
